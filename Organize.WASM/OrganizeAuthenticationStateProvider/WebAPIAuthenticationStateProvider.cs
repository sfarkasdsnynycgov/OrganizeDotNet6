...
namespace Organize.WASM.OrganizeAuthenticationStateProvider
{
    public class WebAPIAuthenticationStateProvider : AuthenticationStateProvider, IAuthenticationStateProvider
    {
        ...
        {
            var authStateTask = CreateAuthenticationState(objUser);
            NotifyAuthenticationStateChanged(authStateTask);
        }

        private async Task<AuthenticationState> CreateAuthenticationState(clsUser objUser)
        {
            await _localStorageService.SetItemAsync("authToken", objUser.Token);
            _currentUserService.CurrentUser = objUser;
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", objUser.Token);

            var handler = new JwtSecurityTokenHandler();
            var token = handler.ReadJwtToken(objUser.Token);
